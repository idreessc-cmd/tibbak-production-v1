...
import { getDoctorContact } from '@/lib/repositories/doctors';
...
export default function DoctorContactWidget({ doctorId, isDirectAllowed, initialWebsite }: ContactWidgetProps) {
  ...
  const handleContactRequest = async (action: 'phone' | 'whatsapp') => {
    setIsLoading(true);
    try {
      // Simulate registering the lead and fetching contacts
      const res = await getDoctorContact(doctorId, action, 'Visitor Patient', '+962790000000');
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm space-y-4">
      <h3 className="text-base font-bold text-slate-800 flex items-center gap-2 mb-2">
        ...
        {isRtl ? 'قنوات التواصل المباشرة' : 'Direct Contact Channels'}
      </h3>

      {/* If Gated Lead Success */}
      {showSuccess ? (
        ...
        <>
          ...
          {phone ? (
            ...
            >
              ...
              <span>{isDirectAllowed ? (isRtl ? 'عرض رقم الهاتف' : 'Show Phone Number') : t('contact_request_btn')}</span>
            </button>
          )}
          ...
          {initialWebsite && (
            ...
            >
              ...
              <span>{isRtl ? 'الموقع الإلكتروني للعيادة' : 'Clinic Website'}</span>
            </a>
          )}
        </>
      )}
